...
class _NotificationsTabState extends State<NotificationsTab> {
            ...
            Expanded(
              child: TabBarView(
                children: [
                  ...
                  _buildMatchesList(firestoreService, colorScheme, textTheme),
                ],
              ),
            ),
          ],
  ...
  Widget _buildMatchesList(FirestoreService firestoreService, ColorScheme colorScheme, TextTheme textTheme) {
    return StreamBuilder<List<ProduceListing>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          debugPrint("Error in MatchSuggestions StreamBuilder: ${snapshot.error}");
          return Center(child: Text('Error fetching match suggestions: ${snapshot.error}'));
        }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'When your requests match available produce, they\'ll appear here.',
                    textAlign: TextAlign.center,
                    style: textTheme.bodyMedium?.copyWith(color: colorScheme.onSurfaceVariant.withOpacity(0.7)),
                  ),
          ...
          children: [
            Padding(
              ...
              child: Row(
                ...
                children: [
                  Text(
                    '${listings.length} Match Suggestions',
            ...
            _buildMatchSuggestionCard(context, listings.length),
            const SizedBox(height: 20),
            const Padding(
              padding: EdgeInsets.symmetric(horizontal: 16.0),
              child: Text(
                "Recently Matched Produce",
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
            ),
            Expanded(
              child: ListView.builder(
                padding: const EdgeInsets.only(bottom: 70.0, left: 8.0, right: 8.0, top: 8.0),
                itemCount: listings.length > 3 ? 3 : listings.length, // Show only top 3
                itemBuilder: (context, index) {
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 2),
                                  Text(
                                    'Farmer: ${listing.farmerName ?? "Unknown"}',
  ...
  Future<void> _handleAccept(BuildContext context, MatchSuggestion suggestion, FirestoreService firestoreService) async {
    try {
      MatchStatus nextStatus = MatchStatus.accepted_by_buyer;
  ...
  Future<void> _handleReject(BuildContext context, MatchSuggestion suggestion, FirestoreService firestoreService) async {
    try {
      await firestoreService.updateMatchSuggestionStatus(
        suggestion.id!,
  ...
  // Add a new method to build the match suggestions card
  Widget _buildMatchSuggestionCard(BuildContext context, int count) {
    final textTheme = Theme.of(context).textTheme;
    final colorScheme = Theme.of(context).colorScheme;
