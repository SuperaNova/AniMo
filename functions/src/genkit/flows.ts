...
console.log("Initializing flows.ts - using direct HTTP calls to Gemini API instead of Genkit.");
...
export const generateMatchSuggestionsFlow = async (input: MatchGenerationInput): Promise<GenkitMatchOutput[]> => {
  ...
  for (const itemPair of itemsToProcess) {
    ...
    const prompt = `You are an AI assistant helping to match fresh produce listings from farmers with requests from buyers. Analyze the following Produce Listing and Buyer Request in detail:
